			...
			<div class="table-responsive">
			  <table class="table">
				<tr  class="info">
					...
					<th class='text-right'>Inicio S/.</th>
                                        <th class='text-right'><font color=red>Usuario Inicia.</font></th>
                                        <th class='text-right'>Entradas <br>S/.</th>
                                        <th class='text-right'>Salidas <br>S/.</th>
                                        <th class='text-right'>Cierre Optimo <br>S/.</th>
                                        <th class='text-right'>Cierre Real <br>S/.</th>
                                        <th class='text-right'><font color=red>Usuario Cierre.</font></th>
                                        <th class='text-right'>Faltante <br>S/.</th>
                                        <th>Acciones</th>
				...
				while ($row=mysqli_fetch_array($query)){
						...
						$fin=$row['cierre'];
                                                ...
                                                if($usuario_cierre>0){
                                                    ...
                                                    $suma1= mysqli_query($con, "SELECT SUM(total_venta) AS total1 FROM facturas  where condiciones=1 and (estado_factura<=3 or estado_factura=5) and activo=1 and ven_com=1 and tienda=$tienda and  (DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fecha2' )");
                                                    $row1= mysqli_fetch_array($suma1);
                                                    $total1 = $row1['total1'];
                                                    
                                                    $suma4= mysqli_query($con, "SELECT SUM(total_venta) AS total4 FROM facturas  where  condiciones=1 and activo=1 and (ven_com=5 or ven_com=3) and tienda=$tienda and  (DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fecha2' )");
                                                    $row4= mysqli_fetch_array($suma4);
                                                    $total4 = $row4['total4'];
                                                    
                                                    $suma2= mysqli_query($con, "SELECT SUM(total_venta) AS total2 FROM facturas  where condiciones=1 and estado_factura=6 and activo=1 and ven_com=1 and tienda=$tienda and  (DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fecha2' )");
                                                    $row2= mysqli_fetch_array($suma2);
                                                    $total2 = $row2['total2'];
                                                    
                                                    $suma3= mysqli_query($con, "SELECT SUM(total_venta) AS total3 FROM facturas  where condiciones=1 and activo=1 and (ven_com=2 or ven_com=4) and tienda=$tienda and  (DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fecha2' )");
                                                    $row3= mysqli_fetch_array($suma3);
                                                    $total3 = $row3['total3'];
                                                    
                                                    $suma5= mysqli_query($con, "SELECT SUM(total_venta) AS total5 FROM facturas  where condiciones=1 and activo=1 and ven_com=6 and tienda=$tienda and  (DATE_FORMAT(fecha_factura, '%Y-%m-%d')='$fecha2' )");
					...
					<input type="hidden" value="<?php echo $entrada;?>" id="entrada<?php echo $id_caja;?>">
						...
						<td align="right"><span class="pull-center">
                                                ...
                                                if($usuario_cierre==0){
                                                    ...
                                                    ?>
                                                    <a href="#" class='btn btn-warning btn-xs' title='Editar inicio caja' onclick="obtener_datos('<?php echo $id_caja;?>');" data-toggle="modal" data-target="#myModal2"><i class="fa fa-pencil"></i>Cerrar</a> 
                                                    <?php
